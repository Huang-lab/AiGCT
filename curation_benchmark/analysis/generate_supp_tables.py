...
Output:
  supp_table_ROC_AUC.xlsx  — one sheet per task per threshold
  supp_table_MWU.xlsx      — one sheet per task per threshold
"""
import pandas as pd
import os
...
print(f"Sheets ({len(roc_sheets)}): {list(roc_sheets.keys())}")
